...
def save():
    ...
    def run_connect():
        ...
        print(f"fancy-frame portal: starting connect_wifi for ssid={ssid!r}", flush=True)
        proc = subprocess.run(
            ["/opt/fancy-frame/scripts/connect_wifi.sh", ssid, password, country],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
        )
        print(
            f"fancy-frame portal: connect_wifi exited rc={proc.returncode}; output:\n{proc.stdout}",
            flush=True,
        )
        ...
        if proc.returncode == 0:
            ...
            print(
                f"fancy-frame portal: reboot command exited rc={reboot_proc.returncode}; output:\n{reboot_proc.stdout}",
                flush=True,
            )
